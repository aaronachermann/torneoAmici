...
is_production = bool(database_url or os.environ.get('RAILWAY_ENVIRONMENT') or os.environ.get('RENDER'))

if is_production:
    secret_key = os.environ.get('SECRET_KEY')
    if not secret_key:
        raise RuntimeError('SECRET_KEY deve essere configurata in produzione.')
    ...
    if not database_url:
        raise RuntimeError('DATABASE_URL deve essere configurata in produzione.')

    app.config['SQLALCHEMY_DATABASE_URI'] = database_url
    app.config['DEBUG'] = False
    print("[POSTGRES] Usando PostgreSQL in produzione")
else:
    app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'dev-secret-key-change-this')
    app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///tournament.db'
    app.config['DEBUG'] = True
    print("Usando SQLite in sviluppo")
...
ADMIN_ENDPOINTS = {
    ...
    'edit_player', 'delete_player', 'groups', 'update_team_order',
    'migrate_team_order', 'migrate_database',
    'all_star_team', 'update_all_star_selection', 'remove_all_star_selection',
    'update_final_rankings', 'migrate_fair_play', 'migrate_all_star_team',
    ...
    'generate_complete_tournament_fixed', 'force_update_playoffs',
    'recalculate_all_points', 'init_db', 'force_create_tables',
    'reset_schedule', 'reset_database', 'reset_scores_only',
    'export_database_csv', 'create_teams_only', 'populate_step_by_step',
    'populate_sample_data_route', 'reset_and_populate', 'insert_all_players',
    'check_registration_points',
}
...
DEV_ONLY_ENDPOINTS = {
    'debug_times', 'test_update_times', 'debug_playoff_teams_current',
    'debug_finals_mapping', 'verify_database_schema',
    'debug_semifinal_numbers', 'debug_playoff_teams', 'debug_match_numbers',
    'debug_matches', 'test_mvp_simple', 'debug_mvp', 'debug_mvp_awards',
    'debug_playoff_progression', 'debug_player_stats', 'verify_data',
    'debug_teams', 'verify_player_stats', 'debug_group_matches',
    'debug_group_standings', 'test_update_playoffs_detailed',
    'debug_quarterfinals_status', 'step_by_step_check',
    'insert_random_results', 'insert_test_scores',
}

PUBLIC_ENDPOINTS = {'static', 'index', 'login', 'register', 'logout', 'tournament_info'}
UNSAFE_METHODS = {'POST', 'PUT', 'PATCH', 'DELETE'}


def wants_json_response():
    """Rileva richieste API per evitare redirect HTML su errori auth."""
...
def require_admin_user():
    auth_response = require_authenticated_user()
    if auth_response:
        return auth_response
    if current_user.is_admin():
        return None
    if wants_json_response():
        return jsonify({'success': False, 'message': 'Privilegi admin richiesti'}), 403
    flash('Accesso negato. Privilegi di amministratore richiesti.', 'danger')
    return redirect(url_for('index'))


def validate_same_origin_request():
    """Mitiga CSRF verificando che le richieste mutanti arrivino dallo stesso host."""
...
def is_safe_redirect_url(target):
    ...
    return (not test_url.netloc or test_url.netloc == ref_url.netloc) and test_url.scheme in ('', 'http', 'https')
...
def enforce_access_policy():
    ...
    csrf_response = validate_same_origin_request()
    if csrf_response:
        return csrf_response

    if endpoint in DEV_ONLY_ENDPOINTS:
        if not app.debug:
            abort(404)
    ...
    if endpoint in LOGIN_ENDPOINTS or request.method in UNSAFE_METHODS:
...
def create_admin_user():
    ...
    if not username or not password:
        print("Admin automatico non creato: configura ADMIN_USERNAME e ADMIN_PASSWORD se serve.")
        return
...
class Player(db.Model):
    ...
    def total_goals(self):
        ...
        return sum(stat.goals for stat in self.match_stats if not stat.is_removed)

    @property
    def total_assists(self):
        """Totale assist in tutte le partite."""
        return sum(stat.assists for stat in self.match_stats if not stat.is_removed)

    @property
    def total_penalties(self):
        """Totale penalità in tutte le partite."""
        return sum(stat.penalties for stat in self.match_stats if not stat.is_removed)

    @property
    def matches_played(self):
        """Presenze effettive, escludendo le partite in cui il giocatore non ha giocato."""
        return sum(1 for stat in self.match_stats if not stat.is_removed)
...
class Match(db.Model):
    ...
    def _get_quarterfinal_descriptions(self, match_number):
        ...
        if self.league == 'Major League':
            descriptions = {
                25: {'team1': '1° gruppo D', 'team2': '2° gruppo C'},  # [OK] CORRETTO
                26: {'team1': '1° gruppo A', 'team2': '2° gruppo B'},  # [OK] CORRETTO
                27: {'team1': '1° gruppo C', 'team2': '2° gruppo A'},  # [OK] CORRETTO
                28: {'team1': '1° gruppo B', 'team2': '2° gruppo D'}   # [OK] CORRETTO
            }
        else:  # Beer League
            descriptions = {
                29: {'team1': '3° gruppo B', 'team2': '4° gruppo A'},  # [OK] CORRETTO
                30: {'team1': '3° gruppo D', 'team2': '4° gruppo C'},  # [OK] CORRETTO
                31: {'team1': '3° gruppo A', 'team2': '4° gruppo D'},  # [OK] CORRETTO
                32: {'team1': '3° gruppo C', 'team2': '4° gruppo B'}   # [OK] CORRETTO
            }
...
class FinalRanking(db.Model):
    ...
    def calculate_final_rankings():
        ...
        try:
            ...
            db.session.commit()
            print(f"[OK] Classifiche finali calcolate: {len(rankings)} posizioni")
            return True

        except Exception as e:
            db.session.rollback()
            print(f"[ERRORE] Errore nel calcolo classifiche finali: {e}")
            return False
...
def update_final_rankings():
    ...
    try:
        ...
        if success:
            flash('[CLASSIFICA] Classifiche finali calcolate con successo!', 'success')
        else:
            flash('[ATTENZIONE] Non è possibile calcolare le classifiche finali. Completa prima tutte le finali.', 'warning')
    except Exception as e:
        flash(f'[ERRORE] Errore nel calcolo delle classifiche finali: {str(e)}', 'danger')
...
def migrate_fair_play():
    ...
    try:
        ...
        db.create_all()
        #flash('[MANUTENZIONE] Migrazione Fair Play completata con successo!', 'success')
    except Exception as e:
        db.session.rollback()
        #flash(f'[ERRORE] Errore durante la migrazione Fair Play: {str(e)}', 'danger')
...
def sync_match_data(match_id):
    ...
    try:
        ...
        # Log per debug
        print(f"[SYNC] Sync match {match_id}: {len(saved_data)} campi ricevuti")
        ...
        if 'team1_score' in saved_data and 'team2_score' in saved_data:
            try:
                ...
                print(f"[ERRORE] Errore conversione punteggi: {e}")
        ...
        for player in all_players:
            try:
                ...
                is_active = saved_data.get(f'player_{player.id}_is_active', '1') in ['1', 1, True, 'true', 'on']
                ...
                is_best_team1 = is_active and saved_data.get('best_player_team1') == str(player.id)
                is_best_team2 = is_active and saved_data.get('best_player_team2') == str(player.id)

                if not is_active:
                    goals = 0
                    assists = 0
                    total_penalty_duration = 0
                    goal_times = []
                    assist_times = []
                    penalty_times = []
                    penalty_durations = []
                ...
                print(f"[ERRORE] Errore statistiche giocatore {player.id}: {e}")
                continue
        ...
        db.session.rollback()
        print(f"[ERRORE] Errore sync match {match_id}: {str(e)}")
        import traceback
        traceback.print_exc()
...
def quick_save_match(match_id):
    ...
    try:
        ...
        if content_type and 'application/json' in content_type:
            ...
            if raw_data:
                try:
                    ...
                    print(f"[ATTENZIONE] Quick save data non JSON: {raw_data[:100]}...")
                    return '', 204
            else:
                data = {}
        ...
        print(f"[URGENTE] Quick save match {match_id}: {len(data)} campi (Content-Type: {content_type})")
        ...
        print(f"[ERRORE] Errore quick save: {e}")
        return '', 500
...
def get_current_match_data(match_id):
    ...
    try:
        ...
        print(f"[ERRORE] Errore get current data: {e}")
        return jsonify({'success': False, 'message': str(e)})
...
def update_dates():
    ...
    try:
        ...
        old_dates = {
            ...
            'quarterfinals_ml_date': settings.quarterfinals_ml_date,
            'quarterfinals_bl_date': settings.quarterfinals_bl_date,
            'semifinals_ml_date': settings.semifinals_ml_date,
            'semifinals_bl_date': settings.semifinals_bl_date,
        ...
        updated_matches = update_existing_schedule_dates(settings, old_dates)
        settings.updated_at = datetime.utcnow()
        db.session.commit()

        flash(f'Date del torneo aggiornate con successo! Aggiornate {updated_matches} partite nel calendario.', 'success')
    except Exception as e:
        db.session.rollback()
...
def update_times():
    ...
    try:
        ...
        if auto_update_calendar:
            ...
            db.session.commit()
            flash(f'[OK] Orari del torneo aggiornati e applicati a {updated_matches} partite del calendario!', 'success')
        else:
            flash('[OK] Orari del torneo aggiornati con successo! Usa il pulsante "Applica al Calendario" per aggiornare le partite esistenti.', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore nell\'aggiornamento degli orari: {str(e)}', 'danger')
        import traceback
...
def update_schedule_times():
    ...
    try:
        ...
        if updated_matches > 0:
            flash(f'[OK] Aggiornati gli orari di {updated_matches} partite nel calendario!', 'success')
        else:
            flash('[INFO] Nessuna partita da aggiornare. Gli orari erano già corretti.', 'info')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore nell\'aggiornamento degli orari del calendario: {str(e)}', 'danger')
        import traceback
...
def debug_times():
    ...
    if settings:
        ...
        print("[ERRORE] NESSUN SETTINGS TROVATO")
    ...
    if settings:
        ...
        try:
            ...
            print(f"[ERRORE] Errore simulazione: {e}")
...
def test_update_times():
    ...
    try:
        settings = TournamentSettings.get_settings()
        if not settings:
            flash('[ERRORE] Settings non trovati', 'danger')
            return redirect(url_for('settings'))
        ...
        flash(f'[OK] Test completato! Aggiornate {updated_count} partite.', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore nel test: {str(e)}', 'danger')
        import traceback
...
def fix_category_field():
    ...
    try:
        ...
        return '''
        <h2>[OK] Campo category aggiornato con successo!</h2>
        ...
        return f'''
        <h2>[ERRORE] Errore nell'aggiornamento</h2>
...
def migrate_penalty_durations():
    ...
    try:
        ...
        if not db.inspect(db.engine).has_table('player_match_stats'):
            flash('[ERRORE] La tabella PlayerMatchStats non esiste.', 'danger')
            return redirect(url_for('index'))
...
def migrate_player_stats_schema():
    ...
    try:
        # Verifica se la tabella PlayerMatchStats esiste
        if not db.inspect(db.engine).has_table('player_match_stats'):
            flash('[ERRORE] La tabella PlayerMatchStats non esiste. Crea prima il database.', 'danger')
            return redirect(url_for('index'))
        ...
        if columns_added:
            flash(f'[OK] Migrazione completata! Aggiunte colonne: {", ".join(columns_added)}', 'success')
        else:
            flash('[INFO] Tutte le colonne sono già presenti nel database.', 'info')

        # Verifica finale
        updated_columns = [col['name'] for col in inspector.get_columns('player_match_stats')]
        flash(f'[INFO] Colonne attuali nella tabella: {", ".join(updated_columns)}', 'info')
...
def verify_database_schema():
    ...
    try:
        ...
        flash(f'[ELENCO] Tabelle nel database: {", ".join(tables)}', 'info')
        ...
        if 'player_match_stats' in tables:
            ...
            flash(f'[DEBUG] Colonne in player_match_stats: {", ".join(column_info)}', 'info')
        else:
            flash('[ATTENZIONE] La tabella player_match_stats non esiste', 'warning')

    except Exception as e:
        flash(f'[ERRORE] Errore nella verifica schema: {str(e)}', 'danger')
...
def fix_complete_playoff_system():
    ...
    try:
        ...
        db.session.commit()
        flash('[PLAYOFF] Sistema playoff completamente riparato! Ora vedrai le descrizioni corrette.', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore durante il fix: {str(e)}', 'danger')
...
def format_date_range_it(start_date, end_date):
    if not start_date or not end_date:
        return ''

    if start_date == end_date:
        return format_date_it(start_date)

    if start_date.year == end_date.year and start_date.month == end_date.month:
        return f"{start_date.day}-{end_date.day} {MONTHS_IT[end_date.month]} {end_date.year}"

    if start_date.year == end_date.year:
        return (
            f"{start_date.day} {MONTHS_IT[start_date.month]} - "
            f"{end_date.day} {MONTHS_IT[end_date.month]} {end_date.year}"
        )

    return f"{format_date_it(start_date)} - {format_date_it(end_date)}"


def get_match_sheet_tournament_context(match=None):
    settings = TournamentSettings.get_settings()
    tournament_name = (
        settings.tournament_name
        if settings and settings.tournament_name
        else 'Torneo Amici dello Skater'
    )

    dates = []
    if settings:
        dates = [
            settings.qualification_day1,
            settings.qualification_day2,
            settings.quarterfinals_ml_date,
            settings.quarterfinals_bl_date,
            settings.semifinals_ml_date,
            settings.semifinals_bl_date,
            settings.finals_date,
        ]
    dates = [date_obj for date_obj in dates if date_obj]

    if not dates and match and match.date:
        dates = [match.date]
    ...
    return {
        ...
        'subtitle': f"Cassina d'Agno, {format_date_range_it(start_date, end_date)}",
    }


def get_player_games_played_counts(player_ids):
    """Conta le presenze effettive dei giocatori, escludendo chi non ha giocato."""
    if not player_ids:
        return {}
...
def get_last_player_jersey_numbers(player_ids, current_match=None):
    """Restituisce l'ultimo numero usato in una partita giocata da ogni giocatore."""
    if not player_ids:
        return {}

    query = db.session.query(PlayerMatchStats, Match).join(
        Match,
        PlayerMatchStats.match_id == Match.id
    ).filter(
        PlayerMatchStats.player_id.in_(player_ids),
        PlayerMatchStats.jersey_number.isnot(None),
        PlayerMatchStats.is_removed != True
    )

    if current_match and current_match.date and current_match.time:
        query = query.filter(
            db.or_(
        ...
        query = query.filter(Match.id != current_match.id)
    ...
    jersey_numbers = {}
    for stats, _match in rows:
        if stats.player_id not in jersey_numbers:
            jersey_numbers[stats.player_id] = stats.jersey_number

    return jersey_numbers


def update_existing_schedule_dates(settings, old_dates=None):
    """Aggiorna le date delle partite esistenti quando cambiano le date settings."""
    updated_matches = 0

    def apply_date(match, new_date):
        nonlocal updated_matches
        if new_date and match.date != new_date:
            match.date = new_date
            updated_matches += 1

    group_matches = Match.query.filter_by(phase='group').order_by(Match.date, Match.time).all()
    for index, match in enumerate(group_matches):
        if old_dates:
            if match.date == old_dates.get('qualification_day1'):
                apply_date(match, settings.qualification_day1)
                continue
            if match.date == old_dates.get('qualification_day2'):
                apply_date(match, settings.qualification_day2)
                continue

        halfway = (len(group_matches) + 1) // 2
        apply_date(match, settings.qualification_day1 if index < halfway else settings.qualification_day2)

    phase_date_map = [
        ('quarterfinal', 'Major League', settings.quarterfinals_ml_date),
        ('quarterfinal', 'Beer League', settings.quarterfinals_bl_date),
        ('semifinal', 'Major League', settings.semifinals_ml_date),
        ('semifinal', 'Beer League', settings.semifinals_bl_date),
        ('final', None, settings.finals_date),
        ('placement', None, settings.finals_date),
        ('third_place', None, settings.finals_date),
    ]

    for phase, league, new_date in phase_date_map:
        query = Match.query.filter_by(phase=phase)
        if league:
            query = query.filter_by(league=league)
        for match in query.all():
            apply_date(match, new_date)
        ...
        if completed_matches:
            ...
            if db.inspect(db.engine).has_table('player_match_stats'):
                ...
                if matches_removed_from > 0:
                    flash(f'[OK] {player_name} aggiunto a {team.name}', 'success')
                    flash(f'[AGGIORNA] Giocatore automaticamente rimosso da {matches_removed_from} partite già completate', 'info')
                else:
                    flash(f'[OK] {player_name} aggiunto a {team.name}', 'success')
            else:
                flash(f'[OK] {player_name} aggiunto a {team.name}', 'success')
                flash('[ATTENZIONE] Sistema statistiche non disponibile - controlla manualmente le partite', 'warning')
        else:
            flash(f'[OK] {player_name} aggiunto a {team.name}', 'success')

        # Check if team exceeds registration points limit
        if team.player_points_total > 20:
            flash('[ATTENZIONE] Attenzione: La squadra supera il limite di 20 punti tesseramento!', 'warning')
...
def fix_existing_players_in_completed_matches(team_id):
    ...
    if not db.inspect(db.engine).has_table('player_match_stats'):
        flash('[ERRORE] Sistema statistiche non disponibile', 'danger')
    ...
    if not completed_matches:
        flash('[INFO] Nessuna partita completata trovata per questa squadra', 'info')
        return redirect(url_for('team_detail', team_id=team_id))
    ...
    if request.form.get('confirm') == 'yes':
        ...
        db.session.commit()
        flash(f'[OK] {total_removed} presenze rimosse da partite completate', 'success')
        return redirect(url_for('team_detail', team_id=team_id))
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if old_name != player.name or old_registered != is_registered or old_category != category:
                flash(f'[OK] Giocatore aggiornato: {old_name} → {player.name} '
                      f'({old_points}pt → {new_points}pt)', 'success')
            else:
                flash(f'[INFO] Nessuna modifica per {player.name}', 'info')

            # Controlla se la squadra supera il limite
            team_total = player.team.player_points_total
            if team_total > 20:
                flash(f'[ATTENZIONE] Attenzione: {player.team.name} ora ha {team_total} punti tesseramento (limite: 20)', 'warning')

        except Exception as e:
            db.session.rollback()
            flash(f'[ERRORE] Errore durante l\'aggiornamento: {str(e)}', 'danger')
...
def delete_player(player_id):
    ...
    try:
        ...
        flash(f'[ELIMINA] Giocatore {player_name} rimosso con successo', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore durante l\'eliminazione: {str(e)}', 'danger')
...
def reset_match(match_id):
    ...
    print(f"[AGGIORNA] RESET MATCH CHIAMATO per partita {match_id}")
    ...
    try:
        ...
        print(f"[INFO] Vecchi punteggi: {old_team1_score}-{old_team2_score}, OT: {old_overtime}, SO: {old_shootout}")
        ...
        if old_team1_score is not None and old_team2_score is not None and match.team1 and match.team2:
            print("[AGGIORNA] Resettando statistiche squadre...")
        ...
        try:
            if db.inspect(db.engine).has_table('player_match_stats'):
                ...
                print(f"[ELIMINA] Eliminate {stats_deleted} statistiche giocatori per partita {match_id}")
        except Exception as e:
            print(f"[ATTENZIONE] Errore eliminazione statistiche giocatori: {e}")
        ...
        db.session.rollback()
        print(f"[ERRORE] ERRORE reset partita: {str(e)}")
        import traceback
        traceback.print_exc()
        flash(f'[ERRORE] Errore durante il reset della partita: {str(e)}', 'danger')
...
def match_detail(match_id):
    ...
    if request.method == 'POST':
        ...
        if team1_score_str and team2_score_str:
            ...
            if hasattr(match, 'overtime'):
                ...
                if overtime_requested and not rules['allow_overtime']:
                    flash(f'[ATTENZIONE] Overtime non permesso in {match.phase}! {rules["description"]}', 'warning')
            ...
            if hasattr(match, 'shootout'):
                ...
                if shootout_requested and not rules['allow_shootout']:
                    flash(f'[ATTENZIONE] Rigori non permessi in {match.phase}! {rules["description"]}', 'warning')
            ...
            if not is_valid:
                flash(f'[ERRORE] {error_msg}', 'error')
                return redirect(url_for('match_detail', match_id=match_id))
        ...
        try:
            ...
            for player in all_players:
                print(f"[AGGIORNA] Aggiornamento statistiche per giocatore {player.name} (ID: {player.id})")
                ...
                is_active = request.form.get(f'player_{player.id}_is_active', '1') == '1'
                ...
                is_best_team1 = is_active and str(player.id) == best_player_team1_id
                is_best_team2 = is_active and str(player.id) == best_player_team2_id
        ...
        # 3. COMMITTA TUTTO
        db.session.commit()
        flash('[OK] Partita salvata completamente: risultato, statistiche e migliori giocatori!', 'success')
    ...
    all_match_players = list(team1_players) + list(team2_players)
    player_ids = [player.id for player in all_match_players]
    last_jersey_numbers = get_last_player_jersey_numbers(player_ids, match)
    player_games_played = get_player_games_played_counts(player_ids)
    ...
    try:
        for player in all_match_players:
            ...
            if stats:
                jersey_number = stats.jersey_number or last_jersey_numbers.get(player.id)
                match_stats[player.id] = {
                    ...
                    'games_played': player_games_played.get(player.id, 0)
                }
                ...
                match_stats[player.id] = {
                    ...
                    'assists': 0,
                    'penalties': 0,
                    'jersey_number': last_jersey_numbers.get(player.id),
                    'goal_times': None,
                    'assist_times': None,
                    'penalty_times': None,
...
def toggle_player_in_match(match_id, player_id):
    ...
    match = Match.query.get_or_404(match_id)
    player = Player.query.get_or_404(player_id)
    print(f"[AGGIORNA] Toggling player {player.name} (ID: {player_id}) in match {match_id}")
    ...
    try:
        ...
        if db.inspect(db.engine).has_table('match'):
            ...
            if migrations_needed:
                ...
                flash(f'[HOCKEY] Sistema Overtime/Rigori abilitato! Aggiunte colonne: {", ".join(migrations_needed)}', 'success')
            else:
                flash('[INFO] Sistema Overtime/Rigori già abilitato.', 'info')
        else:
            # Se la tabella non esiste, creala
            db.create_all()
            flash('[HOCKEY] Tabella Match creata con sistema Overtime/Rigori!', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore durante la migrazione Overtime: {str(e)}', 'danger')
...
def recalculate_all_points():
    ...
    try:
        ...
        db.session.commit()
        flash(f'[PUNTI] Punti ricalcolati per {len(completed_matches)} partite usando il sistema hockey!', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore durante il ricalcolo: {str(e)}', 'danger')
...
def test_mvp_simple():
    ...
    try:
        ...
        flash(f'[DEBUG] MVP Team1: {len(team1_mvps)}, MVP Team2: {len(team2_mvps)}', 'info')

        # Test della funzione corretta
        awards = get_best_player_awards()
        flash(f'[CLASSIFICA] Awards trovati: {len(awards)}', 'success')

        for player, count in awards:
            flash(f' {player.name} ({player.team.name}): {count} MVP', 'success')

    except Exception as e:
        flash(f'[ERRORE] Errore: {str(e)}', 'danger')
...
def update_player_stats(match_id):
    ...
    try:
        ...
        for player in all_players:
            ...
            is_best_team1 = (is_active and str(player.id) == best_player_team1_id and player in team1_players)
            is_best_team2 = (is_active and str(player.id) == best_player_team2_id and player in team2_players)
...
def migrate_all_star_team():
    ...
    try:
        ...
        db.session.rollback()
        flash(f'[ERRORE] Errore durante la migrazione All Star Team: {str(e)}', 'danger')
...
def migrate_add_is_removed():
    ...
    if request.method == 'GET':
        ...
        <div style="max-width: 600px; margin: 50px auto; padding: 20px; font-family: Arial;">
            <h2>[MANUTENZIONE] Migrazione Database - Campo is_removed</h2>
        ...
        if inspector.has_table('player_match_stats'):
            ...
            if 'is_removed' not in columns:
                ...
                with db.engine.connect() as conn:
                    ...
                    conn.commit()
                flash('[OK] Colonna is_removed aggiunta con successo alla tabella player_match_stats!', 'success')
            else:
                flash('[INFO] La colonna is_removed esiste già nella tabella player_match_stats.', 'info')
        else:
            # Se la tabella non esiste, creala
            db.create_all()
            flash('[OK] Tabella player_match_stats creata con successo!', 'success')
            ...
            if inspector.has_table('player_match_stats'):
                ...
                if 'is_removed' not in columns:
                    with db.engine.connect() as conn:
                        conn.execute(db.text('ALTER TABLE player_match_stats ADD COLUMN is_removed BOOLEAN DEFAULT 0'))
                        conn.commit()
                    print("[OK] Colonna is_removed aggiunta con successo!")
                else:
                    print("[INFO] La colonna is_removed esiste già.")
            else:
                print("[ERRORE] Tabella player_match_stats non trovata.")
                print("[SUGGERIMENTO] Esegui prima db.create_all() per creare le tabelle.")
...
def migrate_final_ranking():
    ...
    try:
        ...
        db.create_all()
        flash('[CLASSIFICA] Tabella final_ranking creata con successo!', 'success')
    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore durante la migrazione final_ranking: {str(e)}', 'danger')
...
def debug_mvp():
    ...
    try:
        if db.inspect(db.engine).has_table('player_match_stats'):
            ...
            flash(f'[DEBUG] Debug MVP - Record trovati: {len(mvp_records)}. Dettagli: {debug_info}', 'info')
        else:
            flash('[ERRORE] Tabella player_match_stats non trovata! Esegui la migrazione.', 'danger')
    except Exception as e:
        flash(f'[ERRORE] Errore debug MVP: {str(e)}', 'danger')
...
@app.route('/force_create_tables', methods=['POST'])
def force_create_tables():
    ...
    try:
        ...
        return f"""
        <h2>[OK] Tabelle create con successo!</h2>
        <p><strong>Tabelle create:</strong> {', '.join(tables)}</p>
        <p><strong>User table exists:</strong> {'user' in tables}</p>
        <p>Admin automatico creato solo se ADMIN_USERNAME e ADMIN_PASSWORD sono configurati.</p>
        ...
        return f"""
        <h2>[ERRORE] Errore nella creazione tabelle</h2>
        <p><strong>Errore:</strong> {str(e)}</p>
        <p><a href="/">← Torna alla Home</a></p>
        """
...
def migrate_from_sqlite():
    ...
    if request.method == 'GET':
        return '''
        <h2>[AGGIORNA] Migrazione da SQLite a PostgreSQL</h2>
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <br><br>
            <button type="submit">[AVVIO] Inizia Migrazione</button>
        </form>
    ...
    try:
        ...
        if not request.form.get('confirm'):
            return "[ERRORE] Devi confermare la migrazione"
        ...
        with tempfile.NamedTemporaryFile(suffix='.db') as temp_file:
            ...
            for table_name in tables_order:
                try:
                    ...
                    if not rows:
                        results.append(f"[ATTENZIONE] {table_name}: 0 record")
                        continue
                    ...
                    db.session.commit()
                    results.append(f"[OK] {table_name}: {migrated_count} record migrati")

                except Exception as e:
                    db.session.rollback()
                    results.append(f"[ERRORE] {table_name}: Errore - {str(e)}")

            sqlite_conn.close()

        return f'''
        <h2>[OK] Migrazione Completata!</h2>
        <h3>Risultati:</h3>
        <ul>
...
def import_from_csv():
    ...
    if request.method == 'GET':
        ...
        <form method="post" enctype="multipart/form-data">
            ...
            <br><br>
            <button type="submit">[IMPORTA] Importa Dati</button>
        </form>
    ...
    try:
        ...
        # Verifica conferma
        if not request.form.get('confirm'):
            return "[ERRORE] Devi confermare l'importazione"
            ...
            with zipfile.ZipFile(temp_file.name, 'r') as zf:
                ...
                if 'teams.csv' in zf.namelist():
                    ...
                    for row in reader:
                        ...
                        count += 1
                    db.session.commit()
                    results.append(f"[OK] Squadre: {count} importate")
                ...
                if 'players.csv' in zf.namelist():
                    ...
                    for row in reader:
                        ...
                        if team:
                            ...
                            count += 1
                    db.session.commit()
                    results.append(f"[OK] Giocatori: {count} importati")
                ...
                if 'matches.csv' in zf.namelist():
                    ...
                    for row in reader:
                        ...
                        if team1 and team2:
                            ...
                            count += 1
                    db.session.commit()
                    results.append(f"[OK] Partite: {count} importate")

        return f'''
        <h2>[OK] Importazione Completata!</h2>
        <h3>Risultati:</h3>
        <ul>
        {''.join([f"<li>{result}</li>" for result in results])}
        </ul>
        ...
        db.session.rollback()
        return f'''
        <h2>[ERRORE] Errore durante l'importazione</h2>
        <p><strong>Errore:</strong> {str(e)}</p>
...
def debug_mvp_awards():
    ...
    try:
        ...
        flash(f'[CLASSIFICA] Debug Awards Function: {debug_info}', 'info')

    except Exception as e:
        flash(f'[ERRORE] Errore nella funzione get_best_player_awards: {str(e)}', 'danger')
        import traceback
...
def get_player_match_stats_summary(player_id):
    ...
    try:
        if db.inspect(db.engine).has_table('player_match_stats'):
            stats = PlayerMatchStats.query.filter(
...
def player_total_goals(player):
    ...
    try:
        if hasattr(player, 'match_stats') and player.match_stats:
            return sum(stat.goals for stat in player.match_stats if not stat.is_removed)
    except:
        pass
...
def player_total_assists(player):
    ...
    try:
        if hasattr(player, 'match_stats') and player.match_stats:
            return sum(stat.assists for stat in player.match_stats if not stat.is_removed)
    except:
        pass
...
def player_total_penalties(player):
    ...
    try:
        if hasattr(player, 'match_stats') and player.match_stats:
            return sum(stat.penalties for stat in player.match_stats if not stat.is_removed)
    except:
        pass
...
def get_player_total_stats():
    ...
    for player in players:
        played_stats = [stat for stat in player.match_stats if not stat.is_removed]
        if played_stats:  # Solo se ha giocato almeno una partita
            player_stats.append({
                'player': player,
                ...
                'matches_played': len(played_stats)
            })
...
#         print("[PLAYOFF] Tutte le qualificazioni completate! Aggiornamento quarti automatico...")
#         try:
#             update_playoff_brackets()
#             print("[OK] Quarti aggiornati automaticamente!")
#         except Exception as e:
#             print(f"[ERRORE] Errore aggiornamento quarti: {e}")
...
#             print(f"[PLAYOFF] Quarti {league} completati! Aggiornamento semifinali automatico...")
#             try:
#                 update_semifinals(league)
#                 print(f"[OK] Semifinali {league} aggiornate automaticamente!")
#             except Exception as e:
#                 print(f"[ERRORE] Errore aggiornamento semifinali {league}: {e}")
...
#             print(f"[PLAYOFF] Semifinali {league} completate! Aggiornamento finali automatico...")
#             try:
#                 update_finals(league)
#                 print(f"[OK] Finali {league} aggiornate automaticamente!")
#             except Exception as e:
#                 print(f"[ERRORE] Errore aggiornamento finali {league}: {e}")
...
def update_team_stats(match, old_team1_score=None, old_team2_score=None, old_overtime=None, old_shootout=None):
    ...
    # Debug info
    print(f"[HOCKEY] {team1.name} {match.team1_score}-{match.team2_score} {team2.name}")
    print(f"   Overtime: {match.overtime}, Rigori: {match.shootout}")
    print(f"   Punti: {team1.name}={team1_points}, {team2.name}={team2_points}")

    # *** AGGIORNAMENTI AUTOMATICI PLAYOFF ***
    print(f"[DEBUG] Controllo aggiornamenti automatici per partita fase: {match.phase}")
...
def all_group_matches_completed():
    ...
    print(f"[DEBUG] Controllo partite gironi incomplete: {incomplete_matches} trovate ")
    return incomplete_matches == 0


@app.route('/check_incomplete_matches', methods=['POST'])
def check_incomplete_matches():
    complete = all_group_matches_completed()
...
def force_update_playoffs():
    ...
    try:
        if all_group_matches_completed():
            update_playoff_brackets()
            flash('[PLAYOFF] Playoff aggiornati con successo!', 'success')
        else:
            flash('[ATTENZIONE] Le qualificazioni devono essere completate prima di aggiornare i playoff.', 'warning')
    except Exception as e:
        flash(f'[ERRORE] Errore durante l\'aggiornamento: {str(e)}', 'danger')
...
def update_playoff_brackets():
    ...
    print("[AGGIORNA] Inizio aggiornamento playoff brackets...")

    # Verifica che tutte le qualificazioni siano completate
    if not all_group_matches_completed():
        print("[ERRORE] Le qualificazioni non sono ancora completate")
        return False
    ...
    for group, teams in group_standings.items():
        print(f"[INFO] Girone {group}: {[f'{t.name}({t.group_points}pts)' for t in teams]}")

    # Verifica che ogni girone abbia almeno 4 squadre
    for group in ['A', 'B', 'C', 'D']:
        if len(group_standings[group]) < 4:
            print(f"[ERRORE] Girone {group} ha solo {len(group_standings[group])} squadre!")
            return False
    ...
    try:
        ...
        print(f"[ERRORE] Errore durante l'aggiornamento playoff: {e}")
        db.session.rollback()
        return False
...
def update_semifinals(league):
    ...
    print(f"[AGGIORNA] Aggiornamento semifinali {league}...")
...
def update_finals(league):
    ...
    print(f"[AGGIORNA] Aggiornamento finali {league}...")
...
def reset_matches():
    ...
    try:
        ...
        db.session.commit()
        flash('[OK] Calendario e statistiche azzerati con successo!', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore nell\'azzeramento: {str(e)}', 'danger')
...
def export_database_csv():
    ...
    try:
        ...
        flash(f'[ERRORE] Errore nell\'export del database: {str(e)}', 'danger')
        return redirect(url_for('settings'))
...
def reset_database():
    ...
    try:
        ...
        if confirmation != 'CANCELLA_TUTTO':
            flash('[ERRORE] Conferma non valida. Scrivi esattamente "CANCELLA_TUTTO" per confermare.', 'danger')
            return redirect(url_for('settings'))
        ...
        flash('[ELIMINA] Database completamente azzerato e ricreato!', 'warning')

    except Exception as e:
        flash(f'[ERRORE] Errore nel reset del database: {str(e)}', 'danger')
...
@app.route('/insert_random_results', methods=['POST'])
    ...
    for player in players:
        ...
        try:
            if hasattr(player, 'match_stats'):
                played_stats = [stat for stat in player.match_stats if not stat.is_removed]
                total_goals_new = sum(stat.goals for stat in played_stats)
                total_assists_new = sum(stat.assists for stat in played_stats)
                total_penalties_new = sum(stat.penalties for stat in played_stats)
                player_info.update({
...
def insert_all_players():
    ...
    try:
        ...
        for team_name, players in players_data.items():
            ...
            print(f"\n[HOCKEY] Aggiungendo giocatori a {team_name}...")

            # Elimina giocatori esistenti
            existing_count = Player.query.filter_by(team_id=team.id).count()
            if existing_count > 0:
                Player.query.filter_by(team_id=team.id).delete()
                print(f"  [ERRORE] Rimossi {existing_count} giocatori esistenti")
            ...
            for name, is_registered, category in players:
                try:
                    ...
                    print(f"  [OK] {name} ({'Tesserato' if is_registered else 'Non tesserato'}, {category or 'N/A'})")
        ...
        for team_name in players_data.keys():
            ...
            if team:
                ...
                print(f"  [INFO] {team.name}: {total_points} punti tesseramento")

                if total_points > 20:
                    errors.append(f"[ATTENZIONE] {team.name} supera il limite di 20 punti ({total_points})")

        flash(f'[OK] Inseriti {total_added} giocatori con successo!', 'success')

        if errors:
            flash(f'[ATTENZIONE] Errori: {"; ".join(errors)}', 'warning')

        flash(f'[INFO] Riepilogo: {" | ".join(teams_stats)}', 'info')
...
def check_registration_points():
    ...
    for team in teams:
        total_points = team.player_points_total
        status = "[OK] OK" if total_points <= 20 else f"[ERRORE] LIMITE SUPERATO ({total_points}/20)"

        results.append(f"{team.name}: {total_points} punti {status}")

    flash(f"[INFO] Punti tesseramento: {' | '.join(results)}", 'info')
    return redirect(url_for('teams'))
...
def export_standings_pdf():
    ...
    try:
        print("[DEBUG] Inizio generazione PDF...")

        # Prepara i dati come nella funzione standings()
        group_standings = get_group_standings()
        print(f"[OK] Group standings caricati: {len(group_standings)} gironi")
        ...
        try:
            if db.inspect(db.engine).has_table('player_match_stats'):
                ...
                print("[INFO] Caricamento marcatori...")
                ...
                for player, total_goals, total_matches, total_assists in top_scorers_query:
                    ...
                    player.display_assists_for_ranking = total_assists
                    top_scorers.append(player)
                print(f"[OK] Marcatori caricati: {len(top_scorers)}")


                print("[INFO] Caricamento assist...")
                ...
                for player, total_assists, total_matches, total_goals in top_assists_query:
                    ...
                    player.display_goals_for_ranking = total_goals
                    top_assists.append(player)
                print(f"[OK] Assist caricati: {len(top_assists)}")

                print("[INFO] Caricamento penalità...")
                # Penalità
                ...
                print(f"[OK] Penalità caricate: {len(most_penalties)}")

            else:
                print("[ATTENZIONE] Fallback al sistema vecchio per player stats")
                # Fallback
            ...
            print(f"[ERRORE] Errore statistiche: {e}")
            top_scorers = []
            top_assists = []
            most_penalties = []
        ...
        # MVP Awards
        try:
            print("[CLASSIFICA] Caricamento MVP Awards...")
            best_player_awards = get_best_player_awards()
            if not best_player_awards:
                best_player_awards = []
            print(f"[OK] MVP Awards caricati: {len(best_player_awards)}")
        except Exception as e:
            print(f"[ERRORE] Errore MVP awards: {e}")
            best_player_awards = []

        # Fair Play
        try:
            print("[FAIR PLAY] Caricamento Fair Play...")
            fair_play_ranking = get_fair_play_ranking()
            if not fair_play_ranking:
                fair_play_ranking = []
            print(f"[OK] Fair Play caricato: {len(fair_play_ranking)}")
        except Exception as e:
            print(f"[ERRORE] Errore Fair Play: {e}")
            fair_play_ranking = []

        # Final Rankings
        try:
            print("[CLASSIFICA] Caricamento Final Rankings...")
            ...
            if db.inspect(db.engine).has_table('final_ranking'):
                ...
                final_rankings = []
            print(f"[OK] Final Rankings caricati: {len(final_rankings)}")
        except Exception as e:
            print(f"[ERRORE] Errore Final Rankings: {e}")
            final_rankings = []
        ...
        try:
            ...
            for selection in selections:
                ...
                if position in all_star_data[category]:
                    all_star_data[category][position] = selection.player
            print(f"[OK] All Star Team caricato: {len(selections)} selezioni")
        except Exception as e:
            print(f"[ERRORE] Errore All Star Team: {e}")
            all_star_data = {
        ...
        print(f"[OK] PDF generato con successo: {filename}")
        ...
        print(f"[ERRORE] ERRORE GENERALE PDF: {e}")
        import traceback
        traceback.print_exc()
        flash(f'[ERRORE] Errore nella generazione del PDF: {str(e)}', 'danger')
        return redirect(url_for('standings'))
    ...
    for group, teams in group_standings.items():
        ...
        for i, team in enumerate(teams, 1):
            ...
            data.append([
                str(i),
                team.name,
                str(team.group_games_played),      # [OK] Solo 3 partite qualificazione
                str(team.group_wins),
                str(team.group_wins_overtime),   # [OK] Sconfitte overtime (1 punto)
                str(team.group_losses_overtime),   # [OK] Sconfitte overtime (1 punto)
                str(team.group_losses),            # [OK] Solo sconfitte qualificazione
                str(team.group_goals_for),         # [OK] Solo gol qualificazione
                str(team.group_goals_against),     # [OK] Solo gol qualificazione
                str(team.group_goal_difference),   # [OK] Solo diff qualificazione
                str(team.group_points)             # [OK] Solo punti qualificazione
            ])
...
def debug_group_matches():
    ...
    flash(f'[DEBUG] Debug partite di gruppo: {debug_info}', 'info')
    return redirect(url_for('schedule'))
...
def debug_group_standings():
    ...
    flash(f'[INFO] Classifiche gironi: {standings}', 'info')
    return redirect(url_for('schedule'))
...
def test_update_playoffs_detailed():
    ...
    try:
        print("[TEST] INIZIO TEST AGGIORNAMENTO PLAYOFF")

        # 1. Verifica qualificazioni
        if not all_group_matches_completed():
            flash("[ERRORE] Qualificazioni non completate", 'danger')
            return redirect(url_for('schedule'))

        print("[OK] Qualificazioni completate")
        ...
        for group in ['A', 'B', 'C', 'D']:
            ...
            standings[group] = teams
            print(f"[INFO] Girone {group}: {[t.name for t in teams]}")

        # 3. Verifica che ogni girone abbia 4 squadre
        for group in ['A', 'B', 'C', 'D']:
            if len(standings[group]) < 4:
                flash(f"[ERRORE] Girone {group} ha solo {len(standings[group])} squadre!", 'danger')
                return redirect(url_for('schedule'))

        print("[OK] Tutti i gironi hanno 4 squadre")
        ...
        flash('[PLAYOFF] Playoff aggiornati con successo! Le squadre sono state assegnate ai quarti di finale.', 'success')
...
def debug_quarterfinals_status():
    ...
    flash(f'[CLASSIFICA] Debug quarti di finale: {debug_info}', 'info')
    return redirect(url_for('schedule'))
...
def step_by_step_check():
    ...
    print("[DEBUG] INIZIO DEBUG all_group_matches_completed()")

    # Passo 1: Ottieni tutte le partite di gruppo
    group_matches = Match.query.filter_by(phase='group').all()
    print(f"[ELENCO] Trovate {len(group_matches)} partite di gruppo")
    ...
    for i, match in enumerate(group_matches):
        ...
        if not is_complete:
            incomplete_count += 1
            print(f"[ERRORE] Partita {i+1} incompleta: {match.team1.name if match.team1 else 'N/A'} ({match.team1_score}) vs {match.team2.name if match.team2 else 'N/A'} ({match.team2_score})")
        else:
            print(f"[OK] Partita {i+1} completa: {match.team1.name} ({match.team1_score}) vs {match.team2.name} ({match.team2_score})")

    # Passo 3: Risultato della funzione
    function_result = all_group_matches_completed()
    manual_result = incomplete_count == 0

    print(f"[CALCOLO] Conteggio manuale partite incomplete: {incomplete_count}")
    print(f"[MANUTENZIONE] Risultato funzione all_group_matches_completed(): {function_result}")
    print(f"[ATTESO] Risultato atteso (manual): {manual_result}")
    print(f"[PLAYOFF] Match: {function_result == manual_result}")
...
@app.route('/insert_test_scores', methods=['POST'])
def insert_test_scores():
    ...
    try:
        ...
        message = f'[RANDOM] Inseriti punteggi casuali in {matches_updated} partite! '
        ...
        if all_group_matches_completed():
            flash('[OK] Tutte le qualificazioni completate! I playoff verranno aggiornati automaticamente.', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore durante l\'inserimento punteggi casuali: {str(e)}', 'danger')
        import traceback
        print(f"Errore dettagliato: {traceback.format_exc()}")
...
def reset_scores_only():
    ...
    try:
        ...
        flash(f'[AGGIORNA] Resettati i punteggi di {matches_reset} partite di qualificazione!', 'success')

    except Exception as e:
        db.session.rollback()
        flash(f'[ERRORE] Errore durante il reset: {str(e)}', 'danger')
